...
# gdpo diagnostics (local only, no .env changes)
export DEBUG_REWARD_FLOW=0
export DEBUG_REWARD_FLOW_LIMIT=0
GDPO_DIAG_DIR=${GDPO_DIAG_DIR:-"log/gdpo_diag/${EXP_NAME}"}
mkdir -p "${GDPO_DIAG_DIR}"
cat > "${GDPO_DIAG_DIR}/run_config.txt" <<EOF
adv_estimator=gdpo
reward_manager=gdpo (reward loop)
reward_loop_workers=${REWARD_NUM_WORKERS}
overlong.enable=${ENABLE_OVERLONG_BUFFER}
overlong.len=${OVERLONG_BUFFER_LEN}
overlong.penalty_factor=${OVERLONG_BUFFER_PENALTY_FACTOR}
EOF
...
python3 -m verl.trainer.main_ppo \
    ...
    reward_model.num_workers=${REWARD_NUM_WORKERS} \
    ...
    custom_reward_function.path=verl/utils/reward_score/rubric_reward/rurbichub_v1_Medical_gdpo.py \
