...
def compute_grpo_outcome_advantage(
    ...
    """
    Compute advantage for GRPO with support for Dense Rewards (Token-level Attribution).
    
    Modified for Token Attribution:
    Instead of summing rewards into a scalar, this computes 'Returns-to-Go' 
    and normalizes advantages at EACH token step within the group.

    Args:
        token_level_rewards: (bs, response_length) - Now contains dense rewards
        response_mask: (bs, response_length)
        index: (np.ndarray) Group indices
        ...
    """
    
    # 1. 计算 Returns-to-Go (倒序累积回报)
    # 如果是稀疏奖励(Sparse)，这里的结果和原版sum一样；
    # 如果是密集奖励(Dense)，这里会计算每个 Token 之后的预期回报。
    gamma = 1.0
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(token_level_rewards.shape[1])):
            ...
            # 处理 mask: 如果是 padding，return 保持 0 (或者截断)
            running_return = running_return * response_mask[:, t]
            returns[:, t] = running_return

        # 2. Group-wise Normalization (针对每个 Token 位置进行归一化)
        advantages = torch.zeros_like(returns)
        
        # 使用字典将同一个 group 的 returns 收集起来
        # id2returns: group_id -> list of tensor [seq_len]
        id2returns = defaultdict(list)
        id2indices = defaultdict(list) # 记录对应的 batch index

        bsz = returns.shape[0]
        for i in range(bsz):
            idx = index[i]
            id2returns[idx].append(returns[i])
            id2indices[idx].append(i)

        for idx in id2returns:
            # stack 形状: [Group_Size, Seq_Len]
            grp_returns = torch.stack(id2returns[idx])
            
            # 计算该 Group 在每个时间步的 Mean 和 Std
            if grp_returns.shape[0] > 1:
                grp_mean = grp_returns.mean(dim=0)
                grp_std = grp_returns.std(dim=0)
            else:
                grp_mean = 0.0
                grp_std = 1.0
            ...
            if norm_adv_by_std_in_grpo:
                grp_adv = (grp_returns - grp_mean) / (grp_std + epsilon)
            else:
                grp_adv = grp_returns - grp_mean
            
            # 将计算好的 Advantage 填回对应的 Batch 位置
            batch_indices = id2indices[idx]
            for j, batch_idx in enumerate(batch_indices):
                advantages[batch_idx] = grp_adv[j]

        # 再次 Mask 确保 Padding 部分为 0
