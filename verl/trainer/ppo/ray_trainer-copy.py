...
class RayPPOTrainer:
    ...
    def _compute_or_extract_reward(
        ...
        MODIFIED: Supports extracting 'dense_reward_tensor' for Token-level Attribution.
        """
        # 1. If rm_scores already exists, extract it directly
        ...
        # 2. Otherwise, compute reward using reward_fn
        ...
        if reward_for_val:
            ...
            # Try to get Dense Rewards (Token-level) if available
            try:
                # Try calling reward_fn with return_dict=True (Supported by your modified NaiveRewardManager)
                output = reward_fn(batch, return_dict=True)
                
                if isinstance(output, dict):
                    reward_tensor = output["reward_tensor"] # Default sparse rewards
                    reward_extra_infos_dict = output.get("reward_extra_info", {})
                    
                    # IF dense rewards exist, OVERWRITE the reward_tensor with it!
                    # This ensures subsequent Advantage calculation uses the dense values.
                    if "dense_reward_tensor" in output:
                         reward_tensor = output["dense_reward_tensor"]
                         
                else:
                    # Fallback if return_dict is ignored/unsupported
                    if isinstance(output, tuple):
                        reward_tensor, reward_extra_infos_dict = output
                    else:
                        reward_tensor = output
                        reward_extra_infos_dict = {}
                        
            except Exception:
                # Fallback to default compute_reward if anything goes wrong
