...
def calculate_score_components(
    ...
    achieved_points = 0.0
    totals = {"rule": 0.0, "llm": 0.0}
    achieved = {"rule": 0.0, "llm": 0.0}
    per_rubric_components: Dict[str, float] = {}

    for idx, (item, result) in enumerate(zip(rubric_items, grading_response_list)):
        points = float(item.points)
        if points <= 0:
            continue
        verifier = item.tags.get("verifier", "llm")
        bucket = "rule" if verifier == "rule" else "llm"
        totals[bucket] += points
        total_possible_points += points
        met = bool(result and result.get("criteria_met", False))
        if met:
            achieved[bucket] += points
            achieved_points += points
        per_rubric_components[f"rubric_{idx + 1}"] = 0.0
    ...
    for idx, (item, result) in enumerate(zip(rubric_items, grading_response_list)):
        ...
        if points <= 0:
            continue
        met = bool(result and result.get("criteria_met", False))
        per_rubric_components[f"rubric_{idx + 1}"] = (points / total_possible_points) if met else 0.0

    total_score = max(0.0, achieved_points / total_possible_points)
    rule_score = achieved["rule"] / totals["rule"] if totals["rule"] > 0 else 0.0
    llm_score = achieved["llm"] / totals["llm"] if totals["llm"] > 0 else 0.0
    return float(total_score), per_rubric_components, float(rule_score), float(llm_score)
...
async def async_grade_single_example(
    ...
    total_score, components, rule_score, llm_score = calculate_score_components(
        rubric_items, grading_response_list
    )
    return {
        "score": total_score,
        "reward_components": components,
        "rule_reward": rule_score,
        "llm_reward": llm_score,
    }
